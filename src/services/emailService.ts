...
const sendEmail = async ({ email, subject, html, text }: any) => {
  try {
...
export const sendCarCredentials = async ({
  email,
  ownerName,
  generatedCode,
  generatedPassword,
...
export const sendLoginVerificationLink = async ({ email, userName, link }) => {
  await sendEmail({
    email,
...
// ── 5. Repair completion ───────────────────────────────────
export const sendRepairCompletionEmail = async ({ email, ownerName }) => {
  await sendEmail({
    email,

...
export const sendRepairDoneNotification = async (carNumber) => {
...
export const sendNeedsCheckNotification = async (carNumber) => {
  const user = await findUserByCarNumber(carNumber);
  if (!user?.fcmToken) return;
...
// @access private
export const sendNotificationToUser = asyncHandler(async (req, res, next) => {
...
// @access private
export const sendNotificationToAllUsers = asyncHandler(async (req, res, next) => {
  const { title, body } = req.body;
  if (!title || !body)
    return next(new apiError(`title and body are required`, 400));
...
export const sendTheReplyOnBookingRequest = async (userId,status) => {
  const user = await User.findById(userId);
  if (!user?.fcmToken) return;
...
export const sendLowQuantityNotification = async (componentName, quantity) => {
  const admins = await User.find({
    role: "admin",
    fcmToken: { $exists: true, $ne: null },
  });

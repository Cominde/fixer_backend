...
// This resolved to <repo>/backups while the script ran from <repo>/scripts. It
// now runs from <repo>/dist/scripts, so anchor to the repo root to keep reading
// backups from the same directory as before.
...
async function listCloudinaryBackups() {
  try {
    ...
    backups.sort((a, b) => (new Date(b.created_at) as any) - (new Date(a.created_at) as any));
...
async function restoreDatabase(backupPublicId = null) {
  ...
  try {
    ...
    if (!fs.existsSync(dbBackupPath)) {
      ...
      if (dirs.length > 0) {
        // Pre-existing bug kept verbatim: `dbBackupPath` is declared `const`, so
        // this line throws `TypeError: Assignment to constant variable` whenever
        // the primary backup directory is missing. Fixing it would change
        // behaviour, which this conversion is not allowed to do.
        // @ts-ignore
...
export = { restoreDatabase, listCloudinaryBackups };

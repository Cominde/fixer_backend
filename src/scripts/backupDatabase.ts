...
// now runs from <repo>/dist/scripts, so anchor to the repo root to keep writing
// backups to the same directory as before.
...
async function cleanupOldBackups() {
  try {
    ...
    backups.sort((a, b) => (new Date(a.created_at) as any) - (new Date(b.created_at) as any));
...
async function backupDatabase() {
  ...
  try {
    ...
    const { backupPath, timestamp, dbName }: any = await executeMongodump();
